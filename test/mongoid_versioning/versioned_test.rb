    ...
    describe 'instance methods' do
      describe '#revise' do
        ...
        describe 'on new document' do
          ...
          describe 'versions' do
            it 'does not store anything' do
              TestDocument::Version.count.must_equal 0
            end
          end

          describe 'current' do
            let(:current) { TestDocument.find(new_document.id) }
        ...
        describe 'on existing document' do
          ...
          describe 'versions' do
            let(:versions) { TestDocument::Version.where(original_id: existing_document.id).desc(:_version) }
          ...
          describe 'current' do
            let(:current) { TestDocument.find(existing_document.id) }
        ...
        describe 'on concurrent updates' do
          before do
            ...
            @user1 = TestDocument.find(@doc.id)
            @user2 = TestDocument.find(@doc.id)
          ...
          let(:result) { TestDocument.find(@doc.id) }
          ...
          describe 'versions' do
            let(:versions) { TestDocument::Version.where(original_id: @doc.id).desc(:_version) }
          ...
          describe 'current' do
            let(:current) { TestDocument.find(@doc.id) }
      ...
      describe 'versions' do
        ...
        describe '#previous_versions' do
          ...
          it 'reverts document _ids' do
            subject.previous_versions.map(&:original_id).uniq.must_equal [subject.id]
          end
        end
